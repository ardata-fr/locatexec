...
.exec$pandoc <- list(exec_file = NULL, version = NULL)

.exec_names <- new.env()
.exec_names$node <- list(
  exec_win = "node.exe",
  exec_osx = "node",
  exec_unix = "node"
)
...
.exec_names$chrome <- list(
  exec_win = "chrome.exe",
  exec_osx = "Google Chrome",
  exec_unix = "chrome"
)
.exec_names$libreoffice <- list(
  exec_win = "soffice.com",
  exec_osx = "soffice",
  exec_unix = "soffice"
)
.exec_names$python <- list(
  exec_win = "python.exe",
  exec_osx = c("python3", "python2", "python"),
  exec_unix = c("python3", "python2", "python")
)
.exec_names$pip <- list(
  exec_win = c("Scripts/pip.exe", "Scripts/pip2.exe", "Scripts/pip3.exe"),
  exec_osx = c("pip3", "pip2", "pip"),
  exec_unix = c("pip3", "pip2", "pip")
)
.exec_names$firefox <- list(
  exec_win = "firefox.exe",
  exec_osx = "firefox",
  exec_unix = "firefox"
)
.exec_names$excel <- list(
  exec_win = "EXCEL.exe",
  exec_osx = "Microsoft Excel",
  exec_unix = NA_character_
)
.exec_names$powerpoint <- list(
  exec_win = "POWERPNT.exe",
  exec_osx = "Microsoft PowerPoint",
  exec_unix = NA_character_
)
.exec_names$word <- list(
  exec_win = "WINWORD.exe",
  exec_osx = "Microsoft Word",
  exec_unix = NA_character_
)
.exec_names$air <- list(
  exec_win = "air.exe",
...
match_version <- function(exec_files, versions, match_version = NULL) {
  ...
  for (i in seq_along(exec_files)) {
    ...
    if (
      (!is.null(match_version) && current_version == match_version) ||
        (is.null(match_version) && current_version > choosen_ver)
    ) {
...
read_registry_app_path <- function(name) {
  ...
  vapply(
    ...
    function(z) {
      ...
      if (inherits(exec_path, "try-error")) {
        NA_character_
      } else if (!is.null(exec_path$Path) && dir.exists(exec_path$Path)) {
...
exec_candidates <- function(sources, exec) {
  ...
  candidates <- candidates[
    !grepl("Microsoft/WindowsApps", candidates, fixed = TRUE)
  ]
  candidates
}
